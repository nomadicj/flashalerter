...
logging.basicConfig(
    handlers=[TimedRotatingFileHandler(f'{log_dir}/{log_file}',
                                       backupCount=10,
                                       when='midnight')],
...
logging.debug('User set to [%s].', flashfood_user)
logging.debug('DB file set to [%s].', db_file)
logging.info('Logging set to [%s].', log_level)
logging.info('Total [%s] users to be notified.', len(pushover_userkeys))
logging.info('Total [%s] stores to be polled.', len(flashfood_store_ids))
logging.debug('StoreIDs; [%s]', flashfood_store_ids)
...
def main():
    ...
    for store in stores:
        for item in store.get_items():
            logging.debug('Item [%s]', item.id)
            item_row = db.select_item(conn, item.id)
            logging.debug('Select returned [%s]', item_row)
            push_string = ''

            if len(item_row) == 1:
                logging.debug(f'{item.id} seen before...')
                if item_row[0][1] != item.name_en:
                    ...
                    db.update_item(conn, [item.name_en, item.discounted_price, item.id])
                    push_string = 'updated.\r'
                else:
                    logging.debug('... and unchanged. Skipping on...')
                    continue
            elif len(item_row) > 1:
                logging.error(f'Primary key constraint violated. Count of rows with {item.id} is {len(item_row)}.')
                continue
            else:
                logging.debug(f'{item.id} not seen before.')
                db.create_item(conn, [item.id, item.name_en, item.discounted_price])
                push_string = '.\r'
            ...
            if item.original_price > 0 or item.original_price == item.discounted_price:
                ...
                pushover.notify(
                    ...
                    f"{item.name_en} {push_string}${item.discounted_price:.2f} - \
                        {item.discounted_percentage:.0f}% discount",
                    item.image)
            else:
